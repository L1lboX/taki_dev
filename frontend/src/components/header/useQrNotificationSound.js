...
const SOUND_STORAGE_KEY = 'qrNotificationSoundMuted'
const QR_NOTIFICATION_SOUND_URL = '/sounds/new-order.mp3'
...
export function useQrNotificationSound() {
  ...
  useEffect(() => {
    ...
    const audio = new Audio(QR_NOTIFICATION_SOUND_URL)
    audio.preload = 'auto'
  ...
  const playFallbackBeep = useCallback(async () => {
    ...
    oscillator.frequency.setValueAtTime(680, now + 0.2)
